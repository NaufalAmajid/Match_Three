...
public class BoardManager : MonoBehaviour
{
    ...
    // mengecek match seluruh tile pada board untuk menghindari kesalahan match
    public List<TileController> GetAllMatches()
    {
        ...
        for(int x = 0; x < size.x; x++)
        {
            for(int y = 0; y < size.y; y++)
            {
                ...
                //pergi ke tile berikutnya jika tidak ditemukan kecocokan
                ...
                {
                    //menambahkan tile yang belum ditambahkan
    ...
    private Dictionary<TileController, int> GetAllDrop()
    {
        ...
        for(int x = 0; x < size.x; x++)
        {
            for(int y = 0; y < size.y; y++)
            {
                ...
                {
                    //proses untuk semua tile di atas tile yang hancur
                    for(int i = y + 1; i < size.y; i++)
                    {
                        ...
                        //jika tile ini sudah ada di daftar drop, tingkatkan rentang dropnya
                        if(droppingTiles.ContainsKey(tiles[x, i]))
                        {
                            droppingTiles[tiles[x, i]]++;
                        }
                        //jika tidak ada dalam daftar drop, tambahkan dengan drop range one
                        else
                        {
